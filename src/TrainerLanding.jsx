...
const css = `
  ...
  .close-btn { background: #f3f4f1; border: 0; width: 40px; height: 40px; border-radius: 50%; font-size: 20px; cursor: pointer; flex: 0 0 auto; }

  @media (max-width: 980px) {
    .reg-container { max-width: calc(100% - 28px); margin: 18px auto; border-radius: 26px; padding: 28px; }
    .reg-grid { grid-template-columns: 1fr; gap: 28px; }
    .time-grid { grid-template-columns: repeat(4, 1fr); }
  }

  @media (max-width: 640px) {
    .reg-container { max-width: 100%; min-height: 100vh; margin: 0; border-radius: 0; padding: 20px; box-shadow: none; }
    .reg-header { align-items: flex-start; }
    .reg-title { font-size: 28px; line-height: 1.04; letter-spacing: -0.05em; }
    ...
    .slots-header { align-items: flex-start; gap: 10px; flex-direction: column; }
    .time-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; }
    .time-btn { padding: 15px 0; border-radius: 16px; font-size: 15px; }
    .form-row { grid-template-columns: 1fr; gap: 10px; }
    .input-field { font-size: 16px; padding: 15px 16px; border-radius: 16px; }
    .submit-btn { padding: 17px; font-size: 15px; }
    .success-box { border-radius: 20px; padding: 22px; }
  }

  @media (max-width: 380px) {
    .reg-container { padding: 16px; }
      ...
      {!isSubmitted ? (
        <form onSubmit={handleSubmit} className="reg-grid">
          ...
          <div>
            ...
            {/* VALANDINIS TINKLELIS (Panaikintas chaotiškas šokinėjimas) */}
        ...
        <div className="success-box">
          ...
          <p>Paslauga: {selectedService.title}<br />Laikas: {selectedTime} val.<br />Sveikatos duomenys sėkmingai perduoti treneriui.</p>
        </div>
      )}
    </div>
  );
